...
class GrassyModule(LightningModule):
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        loss = self.alpha * model_outputs["train/scattering_reconstruction_loss"] + (1-self.alpha)*model_outputs["train/property_prediction_loss"]
        ...
        self.log("train/scattering_reconstruction_loss", 
                 ...
                 on_step=False, on_epoch=True, prog_bar=True, batch_size=batch.y.size(0))
        
        self.log("train/property_prediction_loss", 
                 (1-self.alpha)*model_outputs["train/property_prediction_loss"], 
                 on_step=False, on_epoch=True, prog_bar=True, batch_size=batch.y.size(0))
        
        self.log("train/loss", total_loss, on_step=False, on_epoch=True, prog_bar=True, batch_size=batch.y.size(0))
        self.log("train/mse", train_mse.mean(), on_step=False, on_epoch=True, prog_bar=True, batch_size=batch.y.size(0))
    ...
    def model_step(
        ...
        scattering_reconstruction_loss = self.scattering_reconstruction_loss(scatter_moments_reconstructed, scattering_moments)
    ...
    def predict_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> Dict[str, Any]:
        ...
        torch.save(latents.to("cpu"), f"saved_tensors_12k_chaffer/latents_{batch_idx}.pt")
        torch.save(props.to("cpu"), f"saved_tensors_12k_chaffer/props_{batch_idx}.pt")
